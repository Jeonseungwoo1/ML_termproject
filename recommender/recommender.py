...
class ItemBasedRecommender:
    ...
    def match_recipes(self, user_id, recommendations):
        print("Recommend for", user_id)
        nameid = recommendations.index.tolist()
        for k in nameid:
            print(self.recipes_df.loc[self.recipes_df['id'] == k, 'name'].tolist())
        print("end")
    ...
    def evaluate(self,test_data, predictions):
        matched_indices = test_data['recipe_id'][test_data['recipe_id'].isin(predictions.index)]
        ...
        y_pred = predictions.loc[matched_indices]
        ...
        self.plot_combined_metrics_bar()
    ...
    def sample_recommendations(self, num_recommendations = 5):
        ...
        for i in range(self.config["item_based"]["num_sample"]):
            ...
            if i == 0:
                ...
                user_rows = self.interactions_df[self.interactions_df['user_id'] == value]
                sample_df = pd.concat([sample_df, user_rows])
            else:
                sample_df = pd.concat([sample_df, user_rows])

            similar_items = self.get_recommendations(value, sample_df, num_recommendations)
        ...
        test_data = self.interactions_df[self.interactions_df['user_id'] == value]
        self.evaluate(test_data, all_similar_items)

        all_similar_items = all_similar_items.head(num_recommendations)
        self.match_recipes(value, all_similar_items)
        return sampled_user_id, all_similar_items
    def plot_combined_metrics_bar(self):
        metrics = ['RMSE', 'MAE']
        values = [
            np.mean(self.rmse_list),
            np.mean(self.mae_list)
        ]
        ...
        # Customize the plot
        plt.title(f"When sample_size = {self.config["item_based"]["sample_size"]} and num_sample = {self.config["item_based"]["num_sample"]}")
        plt.ylabel("Value")
